...
# 支持相对导入和直接运行
try:
    from .parsers import get_parser, detect_language, LANGUAGE_CONFIG
...
class CallGraphAnalyzerOptimized:
    ...
    def __init__(self, db_path: str = "call_graph.db", num_workers: Optional[int] = None):
    ...
    def _collect_source_files(self, project_path: Path, exclude_dirs: List[str]) -> List[str]:
        """收集所有源代码文件"""
        source_files = []
    ...
    def _parallel_extract_functions(self, source_files: List[str], show_progress: bool = True) -> List[List[Dict]]:
        """
        并行提取函数定义
        """
        ...
        with Pool(processes=self.num_workers) as pool:
            if show_progress:
                # 使用 imap 可以显示进度
                processed = 0
                for result in pool.imap_unordered(_process_file_functions, source_files, chunksize=10):
    ...
    def _parallel_extract_calls(self, source_files: List[str], show_progress: bool = True) -> List[List[Dict]]:
        """
        并行提取调用关系
        """
        total = len(source_files)
        ...
        with Pool(processes=self.num_workers) as pool:
            if show_progress:
                processed = 0
                for result in pool.imap_unordered(_process_file_calls, args_list, chunksize=10):
                    results.append(result)
                    processed += 1
                    if processed % 50 == 0 or processed == total:
    ...
    def _batch_insert_symbols(self, symbols: List[Dict], batch_size: int, show_progress: bool = True):
        """
        批量插入符号到数据库
        """
        total = len(symbols)
    ...
    def _batch_insert_calls(self, calls: List[Dict], batch_size: int, show_progress: bool = True):
        """
        批量插入调用关系到数据库
        """
        total = len(calls)
    ...
    def _print_progress(self, current: int, total: int, task: str):
        ...
        print(f"\r{task}: [{bar}] {current}/{total} ({percent:.1f}%)", end='', flush=True)
    ...
    def query_full_call_paths(self, function_name: str, max_depth: int = 10, max_paths: int = 1000) -> Dict[str, Any]:
        """查询函数的完整调用路径（向上+向下）"""
