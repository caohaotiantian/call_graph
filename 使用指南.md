# Call Graph Analyzer - 完整使用指南

本文档提供 Call Graph Analyzer 的详细使用说明、最佳实践和实际案例。
...
1. process_order(/path/to/order.py:45)
   调用位置: /path/to/order.py:67
2. update_invoice(/path/to/invoice.py:78)
   调用位置: /path/to/invoice.py:92
```
...
- 自动去重：相同函数只显示一次
- 多次调用标注：显示 `(共 N 处调用)`
- 默认显示函数定义位置：`函数名(文件:行号)`
- verbose 模式显示所有调用点的详细位置
...
- 自动去重：相同函数只显示一次
- 多次调用标注：显示 `(共 N 处调用)`
- 显示被调用函数的定义位置：`函数名(文件:行号)`
- 外部函数（如标准库函数）标记为 `(external)`

#### 查询调用链

```bash
# 查询从 main 函数开始的调用链，深度为 5
...
- 自动去重：相同调用链只显示一次
- 显示统计信息：`找到 N 条唯一调用链 (已去除 M 条重复)`
- 每个函数都显示完整信息：`函数名(文件:行号)`
- 可以清楚地看到整个调用链的代码位置

#### 查询完整调用路径

查询包含目标函数的完整调用路径（从入口到叶子）：
...
1. process_data (function) - /path/to/data.py:34
   签名: def process_data(input_data: Dict[str, Any]) -> Dict[str, Any]

2. process_order (function) - /path/to/order.py:45
   签名: def process_order(order_id: int) -> bool

3. process_payment (function) - /path/to/payment.py:67
   签名: def process_payment(amount: float, method: str) -> str
```
...
- < 500 文件 → `--batch-size 50`
- 500-2000 文件 → `--batch-size 100`
- 2000-5000 文件 → `--batch-size 150-200`
- > 5000 文件 → `--batch-size 200-300`
...
| 项目规模 | 文件数 | 函数数 | 标准模式 | 优化模式 | 提升 |
| -------- | ------ | ------ | -------- | -------- | ---- |
| 小型     | 100    | 500    | 15 秒    | 10 秒    | 1.5x |
| 中型     | 500    | 2000   | 2 分钟   | 30 秒    | 4x   |
| 大型     | 2000   | 12000  | 15 分钟  | 3 分钟   | 5x   |
| 超大型   | 5000+  | 30000+ | 30+分钟  | 6 分钟   | 5-7x |

---

## 实际案例

### 案例 1: 分析 Python Web 项目

```bash
# 1. 分析 Django/Flask 项目
...
for caller in callers:
    print(f"{caller['caller_name']} 调用了 my_function")
...
print(f"处理速度: {stats['files_per_second']:.1f} 文件/秒")
...
**A**: 目前不支持语言过滤，但可以通过排除其他语言的目录实现：

```bash
# 只分析 Python 代码（排除其他语言的目录）
...
1. 使用 `--clear` 重新分析（删除旧数据）
2. 使用更具体的 `--exclude` 排除不必要的目录
3. 为不同模块使用不同数据库
...
```bash
# .gitlab-ci.yml 或 .github/workflows/analyze.yml
...
**A**: 是的，完全支持。分析器会提取所有文件中的函数定义和调用，并建立跨文件的调用关系。

### Q8: 如何处理同名函数？

**A**: 数据库中存储了完整的文件路径和行号，可以区分不同文件中的同名函数。
